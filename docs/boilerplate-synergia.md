...
Para um detalhamento completo consulte [`architecture.md`](architecture.md).
...
A criação e modificação de módulos são tarefas primárias do Agente de IA integrado. Ao solicitar, por exemplo, "Criar um módulo para gerenciar 'Produtos' com campos 'nome', 'preço' e 'descrição'", o Agente **executará diretamente** as seguintes ações: gerar a estrutura de pastas, definir schemas, APIs, configurações e registrar o módulo nos arquivos centrais. Para detalhes sobre como o Agente opera e os arquivos que ele modifica, consulte o [`ai-agent-guide.md`](../ai-agent-guide.md) e para a estrutura detalhada que ele segue, veja [`ai_agent/module_creation_standard.md`](../ai_agent/module_creation_standard.md).

Para um entendimento completo do processo de geração e como customizar um módulo após sua criação, consulte:

- [`module-generator.md`](module-generator.md): Descreve o processo de geração de módulos pelo Agente de IA.
- [`customizing-modules.md`](customizing-modules.md): Guia para ajustar e personalizar módulos.
- [`ai-agent-guide.md`](ai-agent-guide.md): Documentação completa sobre como interagir com o Agente de IA para esta e outras tarefas.
...
Consulte [`ui-customization.md`](ui-customization.md) para um guia completo sobre personalização da UI.
...
*   **Entender a fundo a criação de módulos:** Leia [`module-generator.md`](module-generator.md) (como o Agente IA cria) e [`ai_agent/module_creation_standard.md`](ai_agent/module_creation_standard.md) (a especificação técnica).
*   **Customizar a UI em detalhes:** Mergulhe em [`ui-customization.md`](ui-customization.md).
*   **Explorar a arquitetura de dados:** Consulte [`architecture.md`](architecture.md).
*   **Precisa de uma funcionalidade específica?** Veja os guias em `docs/` para SSR, offline, anexos, etc.
*   **Preparando para produção?** Confira o novo [`deployment-guide.md`](deployment-guide.md).
*   **Fluxos de desenvolvimento comuns?** Veja [`typical-developer-workflows.md`](typical-developer-workflows.md).
...
- [`architecture.md`](architecture.md) – detalhes da camada de dados.
- [`customizing-modules.md`](customizing-modules.md) – passos para personalizar o módulo recém-criado.
- [`attachments.md`](attachments.md) – gerenciamento de uploads com `ostrio:files`.
- [`analytics.md`](analytics.md) – como capturar eventos de navegação e métodos.
- [`coding-patterns.md`](coding-patterns.md) – resumo de padrões de código e arquitetura.
